...
public class RetractAndPulseCommand extends SequentialCommandGroup {

    private static Distance currentSetpoint =
    ...
    super(
      ...
      Commands.repeatingSequence(
        ...
        Commands.waitSeconds(IntakeConstants.Extension.WAIT_TIME.in(Seconds)),
            hopper.setSetpoint(() ->
                currentSetpoint.plus(
                    Inches.of(IntakeConstants.Extension.EXTEND_INCREMENT.in(Inches))
                )
            ),
            Commands.waitSeconds(IntakeConstants.Extension.WAIT_TIME.in(Seconds)),
            Commands.runOnce(() ->
                currentSetpoint = currentSetpoint.minus(
                    Inches.of(IntakeConstants.Extension.RETRACT_INCREMENT.in(Inches))
                )
            )
      ).until(() -> intake.getPosition().in(Inches) <=0.02)
